...
- Import standards on demand:
  ...
  @./.claude/rules/testing.md`,
    ...
    load: { when: 'start', title: 'Session start — always in context before the first keystroke, pulling its @imports with it.' },
  },
  agentsmd: {
    lang: 'md',
    code: `# AGENTS.md
...
Validate input with zod before touching the database.`,
    ...
    load: { when: 'path', chip: 'on edit', title: 'Loads only when Claude edits a file matching paths: src/api/** — otherwise it never enters the window.' },
  },
  dirClaude: {
    lang: 'md',
...
plus at least two edge cases.`,
  ...
  "hooks": {
    ...
    load: { when: 'start', title: 'Read at session start to register the hooks \u2014 the PreToolUse guard then fires on every matching tool call.' },
  },
  mcp: {
    lang: 'json',
    code: `{
...
- Commit messages: imperative mood, <= 72 chars.`,
    ...
    load: { when: 'start', title: 'Session start, before the project files \u2014 the broadest scope reads first, then project config layers on top.' },
  },
  userSettings: {
    lang: 'json',
    code: `{
  "hooks": {
    ...
    load: { when: 'start', title: 'Read at session start alongside the project settings \u2014 its hooks register for every repo you open.' },
  },
  userSkill: {
    lang: 'md',
    code: `---
